...
**Arguments**
  ...
  - top - Returns the first part of the interaction matrix and resulting plot. Similar to head() function.
...
* label - if `label = TRUE` then the interaction strength values (i.e., edge values) are displayed on the plot.

* minInt - Minimum value to display for the interaction strength legend. The default is `minInt = 0`.

* maxInt - Maximum value to display for the interaction strength legend. The default is set to the maximum interaction strength value calaculted from the data.
...
* labelNudge - A value, set by the user, to determine the y_postioning of the variables names. A higher value will postion the label farther above the nodes. the default is set to: `labelNudge = 0.05`.
