...
For the following examples we set the number of features to equal 10 and the number of samples is set to 200 and fit an `mlr3` `ranger` random forest model with $y$ as the response. As the features $x_1$ to $x_5$ are the only variables in the model, $x_6$ to $x_{10}$ are noise variables. As can be seen by the above equation, the only interaction is between $x_1$ and $x_2$
  

Create  the data:
```{r, messages = FALSE}
myData <- genFriedman(noFeatures = 10, noSamples = 200, sigma = 1, bins = NULL, seed = NULL)
```
...
set.seed(1701)
myMatrix <- vividMatrix(task = fr_task, model = fr_mod, gridSize = 20,                                normalize = FALSE, seed = NULL,
