...
The data used in the following examples is simulated from the Friedman benchmark problem 1^[Friedman, Jerome H. (1991) Multivariate adaptive regression splines. The Annals of Statistics 19 (1), pages 1-67.] using thw `genFriedman()` function. This benchmark problem is commonly used for testing purposes. The output is created according to the equation:
...
For the following examples we set the number of features to equal 10 and the number of samples is set to 1000 and fit an `mlr3` `ranger` random forest model with $y$ as the response. As the features $x_1$ to $x_5$ are the only variables in the model, $x_6$ to $x_{10}$ are noise variables. As can be seen by the above equation, the only interaction is between $x_1$ and $x_2$
  

Create  the data:
```{r, messages = FALSE}
myData <- genFriedman(noFeatures = 10, noSamples = 1000, sigma = 1, bins = NULL, seed = NULL)
```
...
set.seed(1701)
myMatrix <- vividMatrix(task = fr_task, model = fr_mod, gridSize = 100,                                normalize = FALSE, seed = NULL,
...
```{r, fig.width = 8, fig.height = 7, eval = F}
set.seed(1701)
...
```{r, fig.width = 8, fig.height = 7, message=F, warning=F, eval = F}
set.seed(1701)
interactionPlot(model =  ab_mod, data = ab, type = "barplot")
```
<center>
