...
## Heatmap Plotting Function -------------------------------------------------------
 intHeatmap <- function(task, model, method = "randomForest_importance",
  ...
  dinteraction <- MatrixFun(task, model)

  # Used for interaction value legend:
  ...
  dinteraction1$varx <- nam[dinteraction1$x]
  dinteraction1$vary <- nam[dinteraction1$y]
  dinteraction1$varx <- factor(dinteraction1$varx, levels=nam)
  dinteraction1$vary <- factor(dinteraction1$vary, levels=rev(nam))
