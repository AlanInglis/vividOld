...
intHeatmap <- function(data, task, model, method = 1, ...){
  ...
  if(method == 1){
    ...
    impMethod = c("ranger_permutation")}
     ...
     impMethod = c("randomForestSRC_importance")
      ...
      (return("Invalid method chosen. Method must be between 1-3. See ?intHeatmap for allowed methods"))
...
# Get Interaction Strength ------------------------------------------------
  ...
  pp <- ggplot(data=dinteraction1, aes(x=varx,y=vary)) + 
    ...
    geom_point(aes(x=Var1,y= rev(Var2),colour=value),size = pointSize, importMatrix %>% filter(importMatrix$Var1==importMatrix$Var2)) +
...
intHeatmap(aq, aqRgrTask, aqMod, method = 3)
intHeatmap(ir, irClasTask, irMod, method = 2)
intHeatmap(FRdf, frRgrTask, frMod, method = 1)
