...
```{r, echo = FALSE}
importanceNet <- function(task, model, method = "randomForest_importance",Threshold = F, Cluster = F,...){
...
If ```Threshold = TRUE``` then only the strongest interactions are displayed, for example:

```{r, fig.width = 8, fig.height = 7}
set.seed(1701)
importanceNet(aqRgrTask, aqMod, method = "randomForest_importance", Threshold = T, Cluster = F)
```
<center>
Fig 2.1: *Network style plot displaying thresholded 2-way interaction strengths between each of the variables and individual variable importance. In this plot the top 5 strongest interactions are displayed*
</center>
